...
import { IListTableCourses } from "@/interface/order.interface";
interface IPropsTable {
  listItem: IListTableCourses[];
  isOrder?: boolean;
}
const TableOrderCourses = ({
  listItem,
  isOrder = false,
}: IPropsTable): JSX.Element => {
      ...
      <table className="min-w-full border-collapse border border-solid border-slate-500 font-medium overflow-hidden">
        <thead className=" border border-solid border-slate-500 rounded-tl-md rounded-tr-md">
          <tr className="bg-slate-700 text-white  ">
            ...
            <th className="border border-solid border-slate-600 px-2 py-4">
              STT
            </th>
            <th className="border border-solid border-slate-600 px-2 py-4">
              Tên Khóa Học
            </th>
            <th className="border border-solid border-slate-600 px-2 py-4 w-20 ">
              Giá
            </th>
          </tr>
        </thead>
        <tbody className=" border border-solid border-slate-500">
          {listItem.map((r: IListTableCourses, index: number) => (
            <tr
            key={index}
              className={`border transition-all duration-150 border-solid border-slate-600 px-2 py-4 border-b ${
                clickedRows[index] ? "bg-slate-200" : "bg-white "
              }`}
              onClick={() => handleRowClick(index)}
            >
              <td className="border border-solid border-slate-600 px-2 py-4 text-center  align-middle">
                {clickedRows[index] ? (
              ...
              <td className="text-center border border-solid border-slate-600 px-2 py-4  align-middle">
                {index + 1}
              </td>
              <td className="border border-solid border-slate-600 px-2 py-4">
                {r.name}
              </td>
              <td className="border border-solid border-slate-600 px-2 py-4 text-center  align-middle">
                $ {r.price}
      ...
      <table className="min-w-full border-collapse border border-solid border-slate-500 font-medium overflow-hidden">
        ...
        <tbody className=" border border-solid border-slate-500">
          ...
          <tr>
            <td
              colSpan={2}
              className="border border-solid border-slate-600 px-2 py-4 text-center"
            >
              Tổng cộng
            </td>
            <td
