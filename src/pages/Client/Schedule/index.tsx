...
import { ISchedule, IScheduleTime } from "@/interface/schedule.interface";
  ...
  return (
    <LayoutMain>
      <div className="mt-32 container">
        <div>
          <h2 className="text-3xl text-black font-normal text-center py-3">
            LỊCH HỌC
          </h2>
        </div>
        <div className="mt-5">
          <div className="grid max-sm:grid-cols-12 gap-3">
            {listSchedule.map((r: ISchedule, index: number) => (
              <>
                <div
                  key={index}
                  className="max-sm:col-span-12 rounded-md p-3 border border-solid border-black"
                >
                  ...
                  <h2 className="flex items-center gap-2">
                    ...
                    <span className="flex gap-2 items-center">
                      <span className="relative flex h-2 w-2">
                        <span
                          className={`h-2 w-2 absolute opacity-75 inline-flex rounded-full ${
                            r.statusType === "inProcess"
                              ? "bg-green-600 animate-ping"
                              : r.statusType === "hasDone"
                                ? "bg-gray-500"
                                : "bg-red-500"
                          }`}
                        ></span>
                  ...
                  <div className="mt-4">
                    <table className="table-fixed w-full border-solid border-black border ">
                      {r.listTime.map((r: IScheduleTime, i: number) => (
                        <tr key={i}>
                          <td className="bg-black font-medium text-white py-2 align-middle text-center text-lg border border-solid border-white">
                            {r.dayOfName}
                          </td>
