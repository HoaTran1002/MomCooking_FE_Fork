...
const SelectLanguage = (): JSX.Element => {
  const languages: IPropsSelectLanguage[] = [
    ...
    { name: "Tiếng Anh", img: UkFlag, type: "en" },
  ...
  const handleChangeLanguage = (index: number, type: string | undefined) => {
    ...
    <div className="box-language h-8 bg-white rounded-sm shadow-lg p-[4px] flex gap-1 relative">
      ...
      >
        {languages.map((r: IPropsSelectLanguage, index: number) => (
          <div
            key={index}
            className={`cursor-pointer hover:bg-[#c0c0c0]  flex text-black items-center gap-1 mb-1 p-1 transition-all duration-300 ${
